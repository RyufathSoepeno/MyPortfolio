    ...
    <header>
        <nav>
            <div class="menu-container">
                ...
                <div class="dropdown">
                    <a href="http://localhost/Web Prog Assignment 5/store.php">Menu</a>
                    <a href="#">Login</a>
                </div>
            </div>
            <div class="logo">
                <p class="bebeklogo"><a href="http://localhost/Web Prog Assignment 5/index.php"><img src="images/Logo.png" alt="Bebek Logo" height="50px" width="50px"></a></p>
            </div>
            <ul class="nav-links">
                <form action="http://localhost/Web Prog Assignment 5/search.php" method="GET" class="search-form">
                ...
                <li><a href="http://localhost/Web Prog Assignment 5/store.php">Menu</a></li>
    ...
    <section class="product-list">
        ...
        <div class="products">
            <?php foreach ($products as $product): ?>
                <div class="product-item" data-pokemon="<?= htmlspecialchars($product[8]) ?>" data-city="<?= htmlspecialchars($product[9]) ?>">
                    <img src="<?= htmlspecialchars($product[7]) ?>" height="90px" width="90px">
                    <p>#<?= htmlspecialchars($product[0]) ?></p>
                    <a href="product.php?id=<?= urlencode($product[0]) ?>&name=<?= urlencode($product[1]) ?>&desc=<?= urlencode($product[2]) ?>&price=<?= urlencode($product[3]) ?>&rating=<?= urlencode($product[5]) ?>&keywords=<?= urlencode($product[4]) ?>&imgSrc=images/products/<?= urlencode($product[0]) ?>.jpg&Pokemon<?= urlencode($product[8]) ?>&Weather<?= urlencode($product[9]) ?>">
